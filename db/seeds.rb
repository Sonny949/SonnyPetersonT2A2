...
post_one.image.attach(io: File.open(Rails.root.join("app", "assets", "images", "shestheculpritii.jpeg")), filename: 'shestheculpritii.jpg' , content_type: "image/jpg")
...
post_two.image.attach(io: File.open(Rails.root.join("app", "assets", "images", "vintage_boat.jpeg")), filename: 'vintage_boat.jpg' , content_type: "image/jpg")
...
post_three.image.attach(io: File.open(Rails.root.join("app", "assets", "images", "oxygen_thief.jpeg")), filename: 'oxygen_thief.jpg' , content_type: "image/jpg")


listing_one = Listing.create(title: "Nascar Dodge Round Track full engine", price: 25000.00, shipment: true, user_id: admin.id, category_id: engine.id, description: "Carby fed Dodge Nascar Engine. 6.0 litre, naturally aspirated, making somewhere between 880 - 900 horsepower. 600 Newton-metres of torque. Email me with any further enquiries. Cash available if you intent to pick this up. As a great man once said 'I wanna go fast!'")
listing_one.images.attach(io: File.open(Rails.root.join("app", "assets", "images", "dodge_covers.jpeg")), filename: 'dodge_covers.jpg' , content_type: "image/jpg")
listing_one.images.attach(io: File.open(Rails.root.join("app", "assets", "images", "nascar_dodge.jpeg")), filename: 'nascar_dodge.jpg' , content_type: "image/jpg")

listing_two = Listing.create(title: "Evercraft shallow vee circuit race hull", price: 25000.00, shipment: false, user_id: editor.id, category_id: hulls.id, description: "Evercraft 17'6\" circuit race hull. Hand laid glass and the last of its kind before the vac pulled glass models. Fitted with safety cell and just had the bottom refurbished. Ready for race set up in either blown or 6.0 litre. Any questions just give me a call after 5pm on weekdays.") 
listing_two.images.attach(io: File.open(Rails.root.join("app", "assets", "images", "hostage_2.jpeg")), filename: 'hostage_2.jpg' , content_type: "image/jpg")
listing_two.images.attach(io: File.open(Rails.root.join("app", "assets", "images", "hostage_reversed.jpeg")), filename: 'hostage_reversed.jpg' , content_type: "image/jpg")

listing_three = Listing.create(title: "Flywheel from Dodge Nascar Engine", price: 1200.00, shipment: true, user_id: user.id, category_id: engine.id, description: "Flywheel from Dodge Nascar round track engine. Good condition. Give me a message or shoot me an email any time for more information.") 
listing_three.images.attach(io: File.open(Rails.root.join("app", "assets", "images", "flywheel.jpeg")), filename: 'flywheel.jpg' , content_type: "image/jpg")

listing_four = Listing.create(title: "5.0l Everingham", price: 22000.00, shipment: false, user_id: admin.id, category_id: boats.id, description: "Multiple championship winning boat! Needs nothing done, just get on the water and race! Hull is a 15'6\" Everingham with a flat back end. Engine is a fully worked 5.0 litre Holden with everything done. Trailer is in great condition and comes with registration. All steering cables replaced, cav gear set to motor, everything balanced. If you need any more information just call or Email me.")
listing_four.images.attach(io: File.open(Rails.root.join("app", "assets", "images", "hot_lemon_egg.jpeg")), filename: 'hot_lemon_egg.jpg' , content_type: "image/jpg")
listing_four.images.attach(io: File.open(Rails.root.join("app", "assets", "images", "hot_lemon_bank.jpeg")), filename: 'hot_lemon_bank.jpg' , content_type: "image/jpg")